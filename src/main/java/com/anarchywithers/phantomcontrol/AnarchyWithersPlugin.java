...
/**
 * AnarchyWithers - Phantom Control
 *
 ...
 */
public final class AnarchyWithersPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("AnarchyWithers phantom control enabled: End-only spawns, endstone/chorus surface required, passive until attacked.");
    }

    @Override
    public void onDisable() {
        getLogger().info("AnarchyWithers phantom control disabled.");
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("anarchywithers")) {
            if (args.length > 0 && args[0].equalsIgnoreCase("reload")) {
                if (!sender.hasPermission("anarchywithers.admin")) {
                    sender.sendMessage("You do not have permission to do that.");
                    return true;
                }
                reloadConfig();
                settings.reload();
                sender.sendMessage("[AnarchyWithers] Configuration reloaded.");
                return true;
            }
            sender.sendMessage("[AnarchyWithers] Usage: /anarchywithers reload");
            return true;
        }
        return false;
    }
